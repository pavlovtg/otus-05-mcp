...
def call_tool(client: httpx.Client, tool_name: str, arguments: dict | None = None) -> dict:
    payload = mcp_request(
        "tools/call",
...
def list_tools(client: httpx.Client) -> dict:
...
def test_list_apis_tool_exists(mcp_client: httpx.Client) -> None:
    """list_apis и search_apis должны быть в списке tools."""
    result = list_tools(mcp_client)
...
def test_list_apis_returns_contracts(mcp_client: httpx.Client) -> None:
    """list_apis должен вернуть непустой список контрактов."""
    result = call_tool(mcp_client, "list_apis")
    assert "result" in result, f"Unexpected response: {result}"
...
def test_search_apis_returns_filtered_results(mcp_client: httpx.Client) -> None:
    """search_apis с запросом должен вернуть результаты."""
    result = call_tool(mcp_client, "search_apis", {"query": "Analytics"})
    assert "result" in result, f"Unexpected response: {result}"
    content = result["result"]["content"]
    assert len(content) > 0, "search_apis returned empty content"


def test_search_apis_empty_query_returns_results(mcp_client: httpx.Client) -> None:
    """search_apis с пустым запросом должен вернуть результаты."""
    result = call_tool(mcp_client, "search_apis", {"query": ""})
    assert "result" in result, f"Unexpected response: {result}"
